#!/usr/bin/env python3
"""
源码安全分析器 — 用小模型检测代码是否存在窃取数据行为
架构: 静态规则提取可疑片段 → 小模型语义判断 → 结构化审计报告

适用场景:
  - 集成到 TEE 流水线，在模型加载/训练前自动审计源码
  - CI/CD 中作为安全门禁
  - 离线环境（无需联网）

推荐模型: Qwen2.5-Coder-0.5B-Instruct (Q4_K_M 量化, 体积更小)
         或 DeepSeek-Coder-1.3B-Instruct

依赖:
  - ollama (本地推理) 或 llama-cpp-python
  - 无需 GPU，CPU 即可运行

用法:
  python3 code_security_analyzer.py <源码目录> [--model qwen2.5-coder:0.5b]
"""
...
# 修复建议映射（与 Go 端 ruleSuggestionMap 保持一致）
RULE_SUGGESTION_MAP = {
...
class Finding:
    ...
    suggestion: Optional[str] = None  # Go 端由 ruleSuggestionMap 生成
...
"""
    ...
    def audit_file_pure_llm(self, file_path: str, max_lines: int = 400) -> dict:
        """对单个源码文件进行纯 LLM 端到端安全审计（不依赖静态规则）"""
        try:
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                lines = f.readlines()
    ...
    def _call_llamacpp(self, prompt: str, num_predict: int = 200) -> dict:
        """通过 llama-cpp-python 直接加载 GGUF 模型"""
        try:
...
def generate_audit_report(
    ...
    """生成结构化的审计报告（与 Go 端 AuditReport 格式一致）"""
    # 按文件分组
