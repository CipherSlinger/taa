	...
	return []Rule{
		...
		},
		{
			...
			Patterns: compilePatterns(
				...
				`logging\.(?:info|debug|warning)\s*\(\s*(?:raw_|train_|test_)?(?:data|dataset|images|samples|batch|x_train|y_train)\b`,
				`sys\.stdout\.write\s*\(\s*(?:raw_|train_|test_)?(?:data|dataset|images|samples|batch|x_train|y_train)\b`,
			),
		},
		{
			ID:          "EMB_004",
