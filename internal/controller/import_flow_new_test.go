...
func TestPhase1TrainingRuntimeConfigWithInOutPlaceholders(t *testing.T) {
	state, server := setupTestServer(t)
	state.mu.Lock()
	state.CurrentPhase = 1
	state.DockerID = "docker-in-out-test"
	state.mu.Unlock()
...
(output_dir / "trained_model.bin").write_text("weights-12345")
(output_dir / "marker.txt").write_text("in-out-success\n")
result = {"dataset": {"total_samples": 10, "splits": {"train": 10, "test": 0}}, "metrics": {"accuracy": 0.99}}
...
`
	...
	dataArchive := buildTarGzArchive(t, map[string]archiveEntry{
		"data/dataset.txt": {
			mode: 0o644,
			data: []byte("feature1,feature2,label\n1,2,0\n"),
		},
	})
	...
	dataResp := postJSON(t, server.URL+"/v1/taa/import", map[string]any{
		"resourceUrl": resourceServer.URL + "/data.tar.gz",
		"requestId":   "req-data-in-out-01",
		"taskId":      "task-in-out-01",
	})
	...
	if string(marker) != "in-out-success\n" {
		t.Fatalf("marker content = %q, want in-out-success\\n", string(marker))
	}
...
func TestPhase1TrainingRuntimeConfigWithDirectModelDirs(t *testing.T) {
	state, server := setupTestServer(t)
	state.mu.Lock()
	state.CurrentPhase = 1
	state.DockerID = "docker-direct-dirs-test"
	state.mu.Unlock()
...
(output_dir / "model.bin").write_text("trained-weights")
(output_dir / "marker.txt").write_text("direct-dirs-success\n")
result = {"dataset": {"total_samples": 5, "splits": {"train": 5, "test": 0}}, "metrics": {"accuracy": 0.95}}
(output_dir / "training_result.json").write_text(json.dumps(result))
`
	modelArchive := buildTarGzArchive(t, map[string]archiveEntry{
		"train.py": {
	...
	dataResp := postJSON(t, server.URL+"/v1/taa/import", map[string]any{
		"resourceUrl": resourceServer.URL + "/data.tar.gz",
		"requestId":   "req-data-direct-01",
		"taskId":      "task-direct-01",
	})
	apiData := decodeResponse(t, dataResp)
	if dataResp.StatusCode != http.StatusOK || apiData.Error != 0 {
		t.Fatalf("import data failed: code=%d msg=%s", dataResp.StatusCode, apiData.Msg)
	...
	// 使用直接路径（不含 <in>/<out> 占位符），模拟模型方配置固定的输入输出路径
	modelInputDir := state.Security.GetModelInputDir()
	modelOutputDir := state.Security.GetModelOutputDir()
	command := fmt.Sprintf("python3 train.py --input %s --output %s", modelInputDir, modelOutputDir)
	...
	expectedResultDir := resultDirForRequestTask(state.Security.ResultDir, "req-data-direct-01", "task-direct-01")
	marker, err := os.ReadFile(filepath.Join(expectedResultDir, "marker.txt"))
	if err != nil {
		t.Fatalf("read marker file: %v", err)
	}
	if string(marker) != "direct-dirs-success\n" {
		t.Fatalf("marker content = %q, want direct-dirs-success\\n", string(marker))
	}

	// 确认训练完成后输入和输出目录中的临时文件已被清理
	inputEntries, err := os.ReadDir(modelInputDir)
	if err != nil {
		t.Fatalf("read modelInputDir after training: %v", err)
	}
	if len(inputEntries) != 0 {
		t.Fatalf("modelInputDir not cleaned up after training: %d entries remaining", len(inputEntries))
	}
	outputEntries, err := os.ReadDir(modelOutputDir)
	if err != nil {
		t.Fatalf("read modelOutputDir after training: %v", err)
	}
