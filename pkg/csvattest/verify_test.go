...
func newTestSM2Key(t *testing.T) *taacrypto.SM2PrivateKey {
	t.Helper()
	key, err := taacrypto.GenerateSM2KeyPair()
	if err != nil {
...
func newTestRootCert(t *testing.T, key *taacrypto.SM2PrivateKey, usage uint32, signer *taacrypto.SM2PrivateKey) []byte {
	t.Helper()
...
func newTestCSVCert(t *testing.T, key *taacrypto.SM2PrivateKey, usage uint32) []byte {
	t.Helper()
...
func signHygonData(t *testing.T, key *taacrypto.SM2PrivateKey, msg []byte, sig []byte) {
	t.Helper()
	r, s, err := taacrypto.SignSM2Signature(key, []byte("test-sm2-user"), msg)
	if err != nil {
...
func putHygonPubKey(dst []byte, pub *taacrypto.SM2PublicKey, userID []byte) {
