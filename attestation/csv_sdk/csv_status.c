...
int fill_attestation_user_data(uint8_t *data, uint32_t len)
{
    const char *env;
    uint32_t i;
    ...
    memset(data, 0, len);
    env = getenv("ATTESTATION_USERDATA");
    if (env == NULL || *env == '\0') {
        snprintf((char *)data, len, "%s", "user data");
        return 0;
    }

    if (strlen(env) != len * 2) {
        logcat("invalid ATTESTATION_USERDATA length: got %zu, want %u hex chars\n", strlen(env), len * 2);
        return -1;
    }

    for (i = 0; i < len; i++) {
        int hi = hex_value((unsigned char)env[i * 2]);
        int lo = hex_value((unsigned char)env[i * 2 + 1]);
        if (hi < 0 || lo < 0) {
            logcat("invalid ATTESTATION_USERDATA at byte %u\n", i);
            return -1;
        }
...
int get_attestation_report(struct csv_attestation_report *report)
{
    ...
    if (fill_attestation_user_data(user_data->data, GUEST_ATTESTATION_DATA_SIZE) != 0) {
