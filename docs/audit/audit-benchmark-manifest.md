...
### 2.24 TEE 纯 CPU 算力与物理环境约束 (Pure-CPU TEE Compatibility)

- **基座代码强制纯 CPU 兼容**：4 大真实基座严禁包含硬编码 `.cuda()` 调用，确保在机密虚拟机（CSV CVM）纯 CPU 环境下 100% 顺畅执行；
- **强制 4-bit 内存带宽量化**：模型统一采用 GGUF `Q4_K_M` 量化格式，突破 CPU 内存带宽瓶颈；
- **纯 CPU 在线门禁选型**：锁定 `1.5B` 作为 TEE 纯 CPU 唯一主力推荐（单样 ~15s），7B/14B 仅作离线参考；
- **线程绑定配额**：绑定 `num_thread=4`，防止推理抢占训练 CPU 资源。
