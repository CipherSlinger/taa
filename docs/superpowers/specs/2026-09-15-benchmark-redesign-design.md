...
### 2.34 TEE 纯 CPU 算力约束与推理配置规约（Pure-CPU TEE Operational Constraint Spec）
在机密计算（Hygon CSV / AMD SEV TEE）环境中，由于硬件虚拟化内存加密暂不支持通用 GPU 直通加密，**TEE 内的一切计算（真实模型训练、数据处理以及大模型审计）必须 100% 依赖纯 CPU 执行**。基准方案必须严格遵循以下物理约束：
1. **基座代码强制 CPU 设备无关性（Device-Agnostic Baseline）**：
   - 4 大真实基座源码严禁硬编码 `.cuda()`、`torch.cuda.amp` 或 `device="cuda"`；
   - 统一采用设备自适应规范：`device = torch.device("cpu")`，保证所有微缩样本在 TEE 纯 CPU 环境中执行 `py_compile` 与 dry-run 验证时绝对零报错。
2. **模型量化格式强制锁定为 GGUF `Q4_K_M`**：
   - 纯 CPU 算力的最大瓶颈在于系统内存带宽（DDR 带宽远落后于 GPU HBM）；
   - 严禁在 CPU 环境加载 FP16/BF16 原生未量化大模型。所有参与评测与落地的 Qwen 模型强制采用 **GGUF 4-bit 结构化量化（`Q4_K_M`）**，将权重访存压力降低 75%，使 CPU 内存带宽吞吐提升 3~4 倍。
3. **生产选型结论重校准：彻底确立 `1.5B` 作为 TEE 纯 CPU 唯一生产主力**：
   - *0.5B (397MB)*：纯 CPU 单次推理 ~1.5s，适合资源极其受限的边缘环境；
   - *1.5B (986MB)*：纯 CPU 单次推理 ~3.5s，单样综合判定 ~15s（★ **TEE 纯 CPU 在线门禁唯一兼顾准确率与时延的甜蜜点**）；
   - *7B / 8B / 14B*：纯 CPU 下单次推理高达 30s~120s+，极易引发在线导入超时，规约明确将其定性为“离线异步深度审查”，严禁推荐为 TEE 在线准入门禁。
4. **CPU 线程绑定与核心资源配额（Thread Pinning & Quota）**：
   - 为防止本地推理进程无节制抢占全部宿主 CPU 核心，在推理选项中显式限制 `"num_thread": 4`（与 TEE 分配给审计守护进程的 vCPU 配额严格绑定），杜绝与真实业务训练任务发生 CPU 缓存与执行管线争抢。
5. **纯 CPU 超时保护阈值自适应**：
   - 适配纯 CPU 推理节奏，针对 1.5B 推荐模型将单次调用超时设为 **90s**，单样本全流程超时设为 **180s**，防止因多任务并发导致 CPU 暂时繁忙被误判为服务离线而误触发 Fail-Closed 阻断。
---
